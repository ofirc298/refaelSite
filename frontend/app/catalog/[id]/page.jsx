...
export default function ProductDetail(){
    ...
    ;(async()=>{
      ...
      try{
        ...
        if(!prod){
          ...
          prod = items.find(x => String(x?.id).toLowerCase() === String(id).toLowerCase()) || null
        }
        if(!off){
          if(!prod) setErr('המוצר לא נמצא')
          else setP(prod)
        }
      }catch{ if(!off) setErr('שגיאה בטעינה') }
  ...
  function addToCart(){
    ...
    try{
      ...
      clearTimeout(window.__detailToast); window.__detailToast=setTimeout(()=>setToast(null), 1200)
    }catch{}
  }
